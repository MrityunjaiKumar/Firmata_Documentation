...
The data communication format uses M\+I\+DI messages. It is not necessarily a M\+I\+DI device, first it uses a faster serial speed, and second, the messages don\textquotesingle{}t always map the same.\hypertarget{md_protocol_protocol_autotoc_md93}{}\doxysection{Message Types}\label{md_protocol_protocol_autotoc_md93}
...
\end{longtabu}
\hypertarget{md_protocol_protocol_autotoc_md94}{}\doxysection{Data Message Expansion}\label{md_protocol_protocol_autotoc_md94}
...
\end{DoxyCode}
\hypertarget{md_protocol_protocol_autotoc_md95}{}\doxysection{Control Messages Expansion}\label{md_protocol_protocol_autotoc_md95}
Set pin mode 
\begin{DoxyCode}{0}
...
\end{DoxyCode}
\hypertarget{md_protocol_protocol_autotoc_md96}{}\doxysection{Sysex Message Format}\label{md_protocol_protocol_autotoc_md96}
...
The full set of core and optional Firmata feature I\+Ds is defined in the \href{https://github.com/firmata/protocol/blob/master/feature-registry.md}{\texttt{ firmata-\/registry.\+md}} file. See the registry for more info on proposing a new feature and obtaining an feature ID.\hypertarget{md_protocol_protocol_autotoc_md97}{}\doxysubsection{Query Firmware Name and Version}\label{md_protocol_protocol_autotoc_md97}
...
\end{DoxyCode}
\hypertarget{md_protocol_protocol_autotoc_md98}{}\doxysubsection{Extended Analog}\label{md_protocol_protocol_autotoc_md98}
...
\DoxyCodeLine{2  END\_SYSEX                (0xF7)}
\end{DoxyCode}
\hypertarget{md_protocol_protocol_autotoc_md101}{}\doxysubsubsection{Capabilities response}\label{md_protocol_protocol_autotoc_md101}
...
\DoxyCodeLine{N  END\_SYSEX                (0xF7)}
\end{DoxyCode}
\hypertarget{md_protocol_protocol_autotoc_md102}{}\doxyparagraph{Supported Modes}\label{md_protocol_protocol_autotoc_md102}
...
{\itshape If no modes are defined for a pin, no values are returned (other than the separator value {\ttfamily 0x7F}) and it should be assumed that pin is unsupported by Firmata.}\hypertarget{md_protocol_protocol_autotoc_md103}{}\doxyparagraph{Mode Resolution}\label{md_protocol_protocol_autotoc_md103}
...
{\itshape For other features (including I2C until updated) the resolution information is less important so a value of 1 is used.}\hypertarget{md_protocol_protocol_autotoc_md104}{}\doxysubsection{Analog Mapping Query}\label{md_protocol_protocol_autotoc_md104}
...
{\itshape The above 2 queries provide static data (should never change for a particular board). Because this information is fixed and should only need to be read once, these messages are designed for a simple implementation in Standard\+Firmata, rather that bandwidth savings (eg, using packed bit fields).}\hypertarget{md_protocol_protocol_autotoc_md105}{}\doxysubsection{Pin State Query}\label{md_protocol_protocol_autotoc_md105}
...
\DoxyCodeLine{N  END\_SYSEX                (0xF7)}
\end{DoxyCode}
\hypertarget{md_protocol_protocol_autotoc_md106}{}\doxysubsection{String}\label{md_protocol_protocol_autotoc_md106}
...
\DoxyCodeLine{N  END\_SYSEX          (0xF7)}
\end{DoxyCode}
\hypertarget{md_protocol_protocol_autotoc_md107}{}\doxysubsection{Sampling Interval}\label{md_protocol_protocol_autotoc_md107}
